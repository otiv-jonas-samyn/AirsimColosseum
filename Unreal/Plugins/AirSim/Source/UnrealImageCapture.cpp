...
/*
UnrealImageCapture::UnrealImageCapture(const common_utils::UniqueValueMap<std::string, APIPCamera*>* cameras, const common_utils::UniqueValueMap<std::string, ASegmentationCamera*>* segmentationCameras)
    : cameras_(cameras), _Segmentation_Cameras_(segmentationCameras)
{
                                              ...
                                              std::vector<msr::airlib::ImageCaptureBase::ImageResponse>& responses, bool use_safe_method) const
{
    ...
    for (unsigned int i = 0; i < requests.size(); ++i) 
    {
        ...
        {
            //This is the place to change stuff before taking the image
