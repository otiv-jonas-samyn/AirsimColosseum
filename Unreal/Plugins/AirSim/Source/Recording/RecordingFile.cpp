...
void RecordingFile::appendRecord(const std::vector<msr::airlib::ImageCaptureBase::ImageResponse>& responses,
                                 msr::airlib::VehicleSimApiBase* vehicle_sim_api, int image_num) const
{
    ...
    for (auto i = 0; i < responses.size(); ++i) 
    {
        ...
        image_file_name << "img_"
                        << response.camera_name << "_" << common_utils::Utils::toNumeric(response.image_type) << "_" << image_num;
        ...
        int image_type = common_utils::Utils::toNumeric(response.image_type);
        if (image_type == 0)
        {
            //Scene view
            image_full_file_path = common_utils::FileSystem::combine(scene_path_, image_file_name.str());
        }
        else if (image_type == 3)
        {
            //Depth view
            image_full_file_path = common_utils::FileSystem::combine(depth_path_, image_file_name.str());
        }
        else if (image_type == 5)
        {
            //Segmentation view
            image_full_file_path = common_utils::FileSystem::combine(segmentation_path_, image_file_name.str());
        }
...
void RecordingFile::startRecording(msr::airlib::VehicleSimApiBase* vehicle_sim_api, const std::string& folder)
{
    try 
    {
        ...
        segmentation_path_ = common_utils::FileSystem::ensureFolder(image_path_, "segmentationFolder");
        scene_path_ = common_utils::FileSystem::ensureFolder(image_path_, "sceneFolder");
        depth_path_ = common_utils::FileSystem::ensureFolder(image_path_, "depthFolder");
