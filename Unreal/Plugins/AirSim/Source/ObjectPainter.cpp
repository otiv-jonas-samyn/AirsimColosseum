...
/*
	...
	Currently doesn't work with nanite enabled meshes
...
FColor GetColorForInstances(int32 classID, int32 instanceID)
{
	...
	int32 smallestValue = instanceID / 255;
    int32 biggestValue = instanceID - (smallestValue * 255);

    // Assuming you have a method to encode an instanceID into G and B values.
    uint8 G = static_cast<uint8>(smallestValue); 
    uint8 B = static_cast<uint8>(biggestValue);


    return FColor(R, G, B, 255); // Alpha set to 255 (fully opaque)
}
...
FColor GetColorFromColorMap(int32 color_index)
{
	...
	int uneven_start = 79;
	int full_start = 149;
	float math = (full_start - uneven_start + 2) / 2;
	int uneven_count = FMath::FloorToInt(math);
	if (color_map_.Num() == 0) {
...
bool UObjectPainter::SetComponentColor(FString component_id, uint32 color_index, TMap<FString, uint32>* name_to_colorindex_map, TMap<FString, UMeshComponent*> name_to_component_map, TMap<FString, FString>* color_to_name_map)
{
    if (name_to_component_map.Contains(component_id)) 
	{
        ...
        if (PaintComponent(actor, color)) 
		{
            ...
            UE_LOG(LogTemp, Log, TEXT("AirSim Segmentation: Adjusted object %s to new ID # %s (color:%s)"), *component_id, *FString::FromInt(color_index), *color_string);
            return true;
        }
        else 
		{
...
bool UObjectPainter::PaintComponent(UMeshComponent* component, const FColor& color)
{
	...
	if (UStaticMeshComponent* staticmesh_component = Cast<UStaticMeshComponent>(component))
	{
		...
		if (staticmesh)
		{
			uint32 num_lod_level = staticmesh->GetRenderData()->LODResources.Num();
			for (uint32 painting_mesh_lod_index = 0; painting_mesh_lod_index < num_lod_level; painting_mesh_lod_index++)
			{
				FStaticMeshLODResources& lod_model = staticmesh->GetRenderData()->LODResources[painting_mesh_lod_index];
				...
				FStaticMeshComponentLODInfo* instance_mesh_lod_info = &staticmesh_component->LODData[painting_mesh_lod_index];
